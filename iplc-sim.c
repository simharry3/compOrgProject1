...
void iplc_sim_LRU_replace_on_miss(int index, int tag)
{
    //If Associativity==1 then only need to update vb and tag no replacement to deal with
    if(cache_assoc==1){
       ...
       cache[index].assoc[0].tag=tag;     
        ...
        int tag_updated=cache[index].replacement[0];
...
void iplc_sim_LRU_update_on_hit(int index, int tag) //int tag used to be int assoc, changed to match replace_on_miss
{
    ...
    cache[index].replacement[cache_assoc-1] = tag;
}
...
int iplc_sim_trap_address(unsigned int address)
{
    ...
    inded = address >> (cache_blockoffsetbits+tag);
    ...
    if(hit==0){
        //incriment cache_miss, use LRU to update vb, tag, etc
        cache_miss++;
        iplc_sim_LRU_replace_on_miss(index,tag);
    }

    //increment cache accesses by 1
    cache_access++; 
...
void iplc_sim_push_pipeline_stage()
{
    ...
    if (pipeline[MEM].itype == LW) {
        int inserted_nop = 0;
        ++instruction_count;
        if(pipeline[MEM].lw.base_reg == pipeline[ALU].rtype.dest_reg){
            inserted_nop = 1;
            pipeline_cycles += 10;
        }
    }
    ...
    if (pipeline[MEM].itype == SW) {
        if(!iplc_sim_trap_address(pipeline[MEM].sw.base_reg)){
...
void iplc_sim_process_pipeline_lw(int dest_reg, int base_reg, unsigned int data_address)
{
    ...
    //Update Instruction Type and Instruction Address
    pipeline[FETCH].itype = LW;
    pipeline[FETCH].instruction_address = instruction_address;
    
	//Update corresponding registries and addresses for lw instruction
...
void iplc_sim_process_pipeline_branch(int reg1, int reg2)
{
    ...
    //Update corresponding registries
